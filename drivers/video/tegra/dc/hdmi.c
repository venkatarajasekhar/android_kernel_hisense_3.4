...
const struct tdms_config tdms_config[] = {
	...
	.drive_current = DRIVE_CURRENT_LANE0(DRIVE_CURRENT_1_500_mA) |
		DRIVE_CURRENT_LANE1(DRIVE_CURRENT_1_500_mA) |
		DRIVE_CURRENT_LANE2(DRIVE_CURRENT_1_500_mA) |
		DRIVE_CURRENT_LANE3(DRIVE_CURRENT_1_500_mA),
	},
	{ /* 720p modes */
	.pclk = 74250000,
	.pll0 = SOR_PLL_BG_V17_S(3) | SOR_PLL_ICHPMP(1) | SOR_PLL_RESISTORSEL |
...
static void tegra_dc_hdmi_detect_worker(struct work_struct *work)
{
	...
	if (hdmi->suspended) {
		dev_info(&dc->ndev->dev, "Suspending. Discard hdmi_detect_worker.\n");
		return;
	}
...
static irqreturn_t tegra_dc_hdmi_irq(int irq, void *ptr)
{
	...
	if (!hdmi->suspended) {
		wake_lock_timeout(&hdmi->wake_lock, HZ / 2);
...
static int tegra_dc_hdmi_init(struct tegra_dc *dc)
{
	...
	res = platform_get_resource_byname(dc->ndev, IORESOURCE_MEM, "hdmi_regs");
	if (!res) {
	...
	wake_lock_init(&hdmi->wake_lock, WAKE_LOCK_SUSPEND, "hdmisleep");
